...
const frontendBaseUrl = "https://black-electronic.vercel.app";
const backendUrl = "https://black-electrisian.onrender.com";
...
async function run() {
  try {
    ...
    app.get("/feedback/:email", async (req, res) => {
      ...
      const email = req.params.email;
      const result = await applyvideoCollection.find({ usersEmail: email }).toArray();
      res.send(result);
    });
    ...
    app.put("/updateStatus/:orderId", async (req, res) => {
      ...
      const result = await applyvideoCollection.updateOne(filter, {
        $set: { code: updateDoc, time: updateTime },
      });
      res.json(result);
    });
    ...
    // get product
    // { page: '0', categories: '', sizing: '', size: '10' }
    app.get("/products", async (req, res) => {
      console.log("🚀 ~ app.get ~ res:", req.query)
      try {
        const page = parseInt(req.query.page) || 0;
        const size = parseInt(req.query.size) || 10;
        let queryparams = {};
    
        // Check if there are queryparams parameters
        if (Object.keys(req.queryparams).length > 2) {
          if (req.query.categories) {
            queryparams.categories = req.query.categories.toLowerCase();
          }
        }
        
        queryparams.categories = queryparams.categories.toLowerCase()
        const query = queryparams.categories
        const cursor = adminProductCollection.find(query);
        const count = await cursor.count();
        const allData = await cursor
          .skip(page * size)
          .limit(size)
          .toArray();
    ...
    app.post("/success", async (req, res) => {
      ...
      res.status(200).redirect(`${frontendBaseUrl}/success/${req.body.tran_id}`);
    ...
    app.post("/fail", async (req, res) => {
      ...
      res.status(400).redirect("${frontendBaseUrl}");
    });
    app.post("/cancel", async (req, res) => {
      // console.log(req.body);
      const order = await paymentCollection.deleteOne({ tran_id: req.body.tran_id });
      res.status(200).redirect("${frontendBaseUrl}/");
    });
    ...
    app.get("/myOrder/:email", async (req, res) => {
      ...
      console.log(req.params.email);
      const email = req.params.email;
      const result = await paymentCollection.find({ cus_email: email }).toArray();
      res.send(result);
    });
    // booking electrician
    app.get("/mybookElectrician/:email", async (req, res) => {
      ...
      console.log(req.params.email);
      const email = req.params.email;
      const result = await bookElectriciansCollection.find({ userEmail: email }).toArray();
      console.log(result);
      res.send(result);
    });
    ...
    app.post("/success", async (req, res) => {
      // console.log(req.body);
      const order = await doctorPaymentCollection.updateOne(
        { tran_id: req.body.tran_id },
        {
          $set: {
            val_id: req.body.val_id,
    ...
    app.post("/validate", async (req, res) => {
      ...
      if (order.val_id === req.body.val_id) {
        const update = await doctorPaymentCollection.updateOne(
          { tran_id: req.body.tran_id },
          {
            $set: {
              paymentStatus: "paid",
    ...
    app.post("/initPost", async (req, res) => {
      const data = {
        ...
        success_url: backendUrl  + "/successdata",
        fail_url: backendUrl  + "/faildata",
        cancel_url: backendUrl  + "/canceldata",
        ipn_url: backendUrl  + "/ipn",
        shipping_method: "Courier",
        product_name: req.body.product_name,
        product_category: "Electronic",
    ...
    app.post("/successdata", async (req, res) => {
      ...
      res.status(200).redirect(`${frontendBaseUrl}/successdata/${req.body.tran_id}`);
    });
